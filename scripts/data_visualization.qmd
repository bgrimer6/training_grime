...
Calculate the daily visits to restoration location
...
- use the 'lab()' function to modify labels
- flip x and y axis
- modify the theme
...
ggplot(data = daily_visits_total,
       aes(x = fct_reorder(restore_loc, n), # this is how you flip order by amount 
           y = daily_visits,
           fill = visitor_type))+
    geom_col()+
    labs(x = "Restoration Location",
  ...
  scale_y_continuous(breaks = seq(0,120,20),
                     expand = c(0,0))+
  scale_fill_viridis_d()  #this is a colorblind friendly palette
```
